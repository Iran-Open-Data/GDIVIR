...
from ..matchmaker import create_one_to_one_mapping
from ..utils import metadata


def export_one_to_one_county_matching() -> None:
    mapping_string = ""
    for year in metadata.geographical_divisions.years[3:]:
        mapping = create_one_to_one_mapping(year, "County")
        ...
        mapping_string += "\n\n"
    file_path = Path("results", "county_one_to_one_mapping.yaml")
    file_path.parent.mkdir(exist_ok=True)
    with file_path.open(mode="w") as file:
        file.write(mapping_string)


def export_one_to_one_mapping_table() -> None:
    mapping_parts: list[pd.DataFrame] = []
    for year in metadata.geographical_divisions.years[3:]:
        mapping_parts.append(
            pd.Series(
                create_one_to_one_mapping(year, "County"),
    ...
    file_path = Path("results", "county_one_to_one_mapping_table.csv")
    file_path.parent.mkdir(exist_ok=True)
